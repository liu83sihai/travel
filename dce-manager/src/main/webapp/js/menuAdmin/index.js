	...
	return {
		init:function(){
			...
			grid=$("#tableGrid").datagrid({
					url: basePath+"/homemng/menuAdmin/listDatas.action",
					height:550,
					rownumbers:true,
					pagination:true,
					fitColumns:true,
		...
		addNew:function () {
            ...
            $('#addDiv').dialog({
                ...
                closed: false,
                cache: false,
                href: basePath+'/homemng/menuAdmin/addMenuDialog.action',
                modal: false,
                toolbar:[
                    {
		...
		deleteMenu:function(id){
			$.ajax({
				url:basePath+"/homemng/menuAdmin/deleteOneResource.action",
				type:"POST",
				data:{id:id},
		...
		saveResource:function(){
			...
			$.ajax({
				url:basePath+"/homemng/menuAdmin/addMenu.action",
				type:"POST",
				data:data,
		...
		editMenu:function(id){
			...
			$('#addDiv').dialog({
				...
				closed: false,
				cache: false,
				href: basePath+'/homemng/menuAdmin/addMenuDialog.action'+(id?"?id="+id:""),
				modal: false,
				toolbar:[
					{
						iconCls:"icon-save",text:"保存",
