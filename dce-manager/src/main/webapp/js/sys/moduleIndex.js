	...
	return {
		init:function(){
			...
			grid=$("#tableGrid").datagrid({
					url:basePath+"/homemng/module/moduleDatas.action",
					height:550,
					rownumbers:true,
					pagination:true,
					fitColumns:true,
		...
		editModule:function(id){
			$('#addDiv').dialog({
				...
				closed: false,
				cache: false,
				href: basePath+'/homemng/module/editModuleDialog.action'+(id?"?moduleId="+id:""),
				modal: false,
				toolbar:[
				{
		...
		deleteModule:function(id){
			...
			$.messager.confirm("消息","确认删除该模块吗，删除后不可恢复",function(r){
			   if(r){
			     $.ajax({
					url:basePath+"/homemng/module/deleteModule/"+id+".action",
					type:"post",
					data:{},
		...
		saveModule:function(){
            ...
            if(!/DK_(.*)/.test(module)){
                $.messager.alert("不符合规则","编码必须是DK_开头");
                return;
            }
			...
			$.ajax({
				url:basePath+"/homemng/module/saveModule.action",
				type:"post",
				data:data,
