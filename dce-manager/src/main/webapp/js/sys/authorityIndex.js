	...
	return {
		init:function(){
			...
			grid=$("#tableGrid").datagrid({
				url: basePath+"/homemng/authority/authoritiesList.action",
				height:550,
				rownumbers:true,
				pagination:true,
				fitColumns:true,
				...
				onDblClickRow:function(index,row){
					$('#setAuthorityDiv').dialog({
						...
						closed: false,
						cache: false,
						href: basePath+"/homemng/authority/bindResources/"+row.id+".action",
						modal: true,
						toolbar:[{
		...
		deleteAuthority:function(id){
			...
			$.messager.confirm("消息","确认删除该权限吗，删除后不可恢复",function(r){
			   if(r){
			     $.ajax({
					url:basePath+"/homemng/authority/deleteOneAuth/"+id+".action",
					type:"post",
					data:{},
		...
		editAuthority:function(id){
			$('#addDiv').dialog({
				...
				closed: false,
				cache: false,
				href: basePath+'/homemng/authority/editAuthority.action'+(id?"?authId="+id:""),
				modal: false,
				toolbar:[
				{
		...
		saveAuthority:function(){
			...
			if(!/DK_ROLE_(.*)/.test(name)){
				$.messager.alert("不符合规则","权限名必须是DK_ROLE_开头");
				return;
			}
			var data={};
			data.name=name;
			...
			$.ajax({
				url:basePath+"/homemng/authority/saveAuthority.action",
				type:"post",
				data:data,
		...
		setRoles:function(id){
			$('#setRolesDiv').dialog({
				...
				closed: false,
				cache: false,
				href: basePath+"/homemng/authority/setRolesIntoAuthorityDialog/"+id+".action",
				modal: true,
				toolbar:[{
					iconCls:"icon-cancel",text:"关闭",
					handler:function(){
