...
public class Generator {
	...
	private void generateNewFileOrInsertIntoFile(IGeneratorModelProvider modelProvider, Configuration config, String templateFile, String outputFilePath)
			throws Exception {
		Template template = config.getTemplate(templateFile);
		if (templateFile.indexOf("jsp") != -1 || templateFile.indexOf("xml") != -1) {
			template.setOutputEncoding(jspEncoding);
		} else {
	...
	private Map getFilePathDataModel(IGeneratorModelProvider modelProvider) throws Exception {
		...
		model.putAll(PropertiesProvider.getProperties()); // generator.properties
	...
	private void saveNewOutputFileContent(Template template, Map model, File outputFile) throws IOException, TemplateException {
		...
		if (fileName.indexOf("jsp") != -1 || fileName.indexOf("xml") != -1) {
			encoding = this.jspEncoding;
		} else {
