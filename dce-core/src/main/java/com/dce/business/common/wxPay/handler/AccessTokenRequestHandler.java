package com.dce.business.common.wxPay.handler;

import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;

import com.dce.business.common.wxPay.client.TenpayHttpClient;
import com.dce.business.common.wxPay.util.JsonUtil;
...
public class AccessTokenRequestHandler extends RequestHandler {
	...
	protected static String getTokenReal() {
		String requestUrl = WXPayConfig.TOKENURL + "?grant_type=" + WXPayConfig.GRANT_TYPE + "&appid="
				+ WXPayConfig.APP_ID + "&secret=" + WXPayConfig.APP_SECRET;
		String resContent = "";
		...
		if (httpClient.call()) {
			...
			if (resContent.indexOf(WXPayConfig.ACCESS_TOKEN) > 0) {
				access_token = JsonUtil.getJsonValue(resContent, WXPayConfig.ACCESS_TOKEN);
			} else {
	...
	private static boolean tokenIsExpire(String access_token) {
		...
		wxReqHandler.setParameter("appid", WXPayConfig.APP_ID);
		wxReqHandler.setParameter("appkey",WXPayConfig.APP_KEY);
		...
		wxReqHandler.setParameter("sign_method", WXPayConfig.SIGN_METHOD);
		String gateUrl = WXPayConfig.GATEURL + access_token;
		wxReqHandler.setGateUrl(gateUrl);

		// 发送请求
		String accesstoken = wxReqHandler.sendAccessToken();
		if (WXPayConfig.EXPIRE_ERRCODE.equals(accesstoken) || WXPayConfig.FAIL_ERRCODE.equals(accesstoken))
			flag = true;
		return flag;
	}
