...
<div class="content" <c:if test="${not empty sessionScope.jdGmapInFlag && sessionScope.jdGmapInFlag ne null}">style="padding-top: 0px;"</c:if>>
	...
	<div class="loginbox">
		<form id="loginForm" action="<c:url value='${basePath}/loanuser/login.do'/>" method="post" >
			...
			<div class="inputbox"><i class="icon i-pwd" id="seePsw"></i><input type="password" placeholder="登录密码"  id="psw"></div>
			<div class="inputbox hasbtn"><i class="icon i-valid"></i><input type="text" placeholder="图片验证码" id="code"><span class="getcode"><img src="${basePath}/imageCode/imageCode.do?pageId=userlogin" title="点击更换验证码" id="codeNum" /></span></div>
		</form>
		...
		<div class="otherbox clearfix"><a href="<c:url value='${basePath }/loanuser/toReg.do?channelCode=${channelCode}'/>" class="fr">立即注册</a><a href="<c:url value='${basePath }/loanuser/toLogin.do'/>" class="fl" id="restPsw">短信登录</a></div>
	</div>
</div>
...
<script type="text/javascript">
	$(function(){
	    ...
	    $("#codeNum").bind("click",function(){
	        $(this).attr("src", "${basePath}/imageCode/imageCode.do?pageId=userlogin&d=" + new Date().getTime());
	    });
...
<script>
	...
	function refCode(isPass){
	    if(true){
	        $("#codeNum").attr("src", "${basePath}/imageCode/imageCode.do?pageId=userlogin&d=" + new Date().getTime());
	    }else{
	        $("#code").attr("src", "${basePath}/imageCode/imageCode.do?pageId=userlogin&d=" + new Date().getTime());
	    }
	}
			...
			if(bPsw){
	            if(bCode){
	            	/* if(!$('.accept').hasClass('yes')){
		                ...
		                var param = {"mobile":mobile,"password":psw,"fromUrl":$('#fromUrl').val() ,"code":code , "pageId":"userlogin"};
		                submitLogin(param);
				   /*  } */
	            }        			
			}
	...
	function submitLogin(param){
	 ...
	 $('#wxLogin').on('click', function(){
			...
			var redirectUrl = "${basePath}/wx/getWxOpenId.do";
